  ...
  return (
    <div className="min-h-screen bg-slate-900 pb-20 text-slate-100">
      <div className="mx-auto max-w-4xl px-6 pt-12 md:pt-16">
        <div className="mb-12">
          <h1 className="text-4xl font-black tracking-tight text-white">{mode === "onboarding" ? "Tailor Your Journey" : "Adjust Your Setup"}</h1>
            ...
            <div className="flex items-center justify-between px-2">
              {STEPS.map((s) => (
                <div key={s.id} className="flex flex-col items-center gap-2">
                  <div
                    className={`flex h-10 w-10 items-center justify-center rounded-full border-2 transition-all duration-500 ${step >= s.id ? "border-green-500 bg-green-500 text-white" : "border-slate-700 bg-slate-800 text-slate-500"}`}
                  >
                    {step > s.id ?
                      <TbCheck className="h-6 w-6" />
                    : <s.icon className="h-5 w-5" />}
                  </div>
                  <span className={`text-[10px] font-bold uppercase tracking-widest ${step >= s.id ? "text-green-400" : "text-slate-500"}`}>{s.title}</span>
                </div>
              ))}
            </div>
            {error && mode === "onboarding" && (
              <div className="mt-4 rounded-xl border border-red-500/20 bg-red-500/10 p-3 text-center text-xs font-bold text-red-400">{error}</div>
            )}
            <div className="relative mt-4 h-1.5 w-full overflow-hidden rounded-full bg-slate-800">
              <div
                className="absolute left-0 h-full bg-green-500 transition-all duration-500 shadow-[0_0_8px_rgba(34,197,94,0.4)]"
        ...
        <div className="flex flex-col gap-8">
          {/* Step 1: Calendar */}
          {isStepVisible(1) && (
            <div className="rounded-3xl border border-slate-800 bg-slate-800 p-8 shadow-sm">
              <div className="mb-6 flex items-center gap-3">
                <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-blue-500/10 text-blue-400">
                  <TbCalendarCheck className="h-7 w-7" />
                </div>
                <div>
                  <h2 className="text-xl font-black text-white">1. Sync with Google Calendar</h2>
                  <p className="text-balance text-sm text-slate-400">We use your calendar to predict spending events before they happen.</p>
                </div>
              </div>
              ...
              {mode === "onboarding" && (
                <div className="mt-6 flex justify-end">
                  <button
                    ...
                    className="group flex items-center gap-2 rounded-xl bg-slate-100 px-6 py-3 text-sm font-bold text-slate-900 transition-all hover:bg-white"
                  >
          ...
          {isStepVisible(2) && (
            <div className="rounded-3xl border border-slate-800 bg-slate-800 p-8 shadow-sm">
              <div className="mb-6 flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-xl font-black text-white">2. Connect Your Bank Accounts</h2>
                  <p className="text-balance text-sm text-slate-400">Securely link your accounts to track live balances and transaction data.</p>
                </div>
              </div>
              ...
              {mode === "onboarding" && (
                <div className="mt-6 flex items-center justify-between">
                  <button
                    onClick={() => setStep(1)}
                    className="flex items-center gap-2 rounded-xl px-4 py-2 text-sm font-bold text-slate-500 transition-colors hover:text-white"
                  >
            ...
            <div className="rounded-3xl border border-slate-800 bg-slate-800 p-8 shadow-sm">
              <div className="mb-8 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h2 className="text-xl font-black text-white">3. Your Outstanding Debts</h2>
                    <p className="text-sm text-slate-400">List all balances you wish to eliminate.</p>
                  </div>
                </div>
              </div>
              ...
              {mode === "onboarding" && (
                <div className="mt-12 flex items-center justify-between">
                  ...
                  <button
                    ...
                    className="group flex items-center gap-2 rounded-xl bg-slate-100 px-8 py-3.5 font-bold text-slate-900 transition-all hover:bg-white"
                  >
                    Define Triggers
          ...
          {isStepVisible(4) && (
            <div className="rounded-3xl border border-slate-800 bg-slate-800 p-8 shadow-sm">
              <div className="mb-6 flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-xl font-black text-white">4. Smart Spending Triggers</h2>
                  <p className="text-sm text-slate-400">Estimates for activities found in your calendar.</p>
                </div>
              </div>
          ...
          {isStepVisible(5) && (
            <div className="rounded-3xl border border-slate-800 bg-slate-800 p-8 shadow-sm">
              <div className="mb-8 flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-xl font-black text-white">5. Income & Allocation Goal</h2>
                  <p className="text-sm text-slate-400">Determine how your surplus funds are distributed.</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 gap-12 lg:grid-cols-2">
                <div className="flex flex-col gap-6">
                  <div>
                    ...
                    <div className="relative">
                      <span className="pointer-events-none absolute inset-y-0 left-4 flex items-center text-lg font-bold text-slate-500">$</span>
                      <input
                        type="number"
                        min={0}
                        value={monthlyIncome || ""}
                        ...
                        className={`w-full rounded-2xl border border-slate-700 bg-slate-900 px-8 py-4 text-2xl font-black text-white outline-none transition-colors focus:border-green-400 focus:ring-4 focus:ring-green-500/10 ${showErrors && monthlyIncome <= 0 ? "border-red-400 bg-red-500/10 focus:ring-red-500/5" : ""}`}
                      />
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        className={`w-full rounded-2xl border border-slate-700 bg-slate-900 px-8 py-4 text-2xl font-black text-white outline-none transition-colors focus:border-green-400 focus:ring-4 focus:ring-green-500/10 ${showErrors && (monthlyLimit < 0 || monthlyLimit > monthlyIncome) ? "border-red-400 bg-red-500/10 focus:ring-red-500/5" : ""}`}
                      />
                    </div>
                  ...
                  {monthlyIncome > 0 && (
                    <div className="mt-4 space-y-4 rounded-2xl bg-slate-900/50 p-6">
                      <div className="flex items-center justify-between text-sm">
                        <span className="font-bold text-slate-400">Total Min. Payments</span>
                        <span className="font-bold text-white">-${totalMinimumPayments.toLocaleString()}</span>
                      </div>
                      <div className="h-px bg-slate-700" />
                      <div className="flex items-center justify-between">
                        <span className="text-sm font-black uppercase tracking-widest text-white">Leftover Flow</span>
                        <span className={`text-xl font-black ${amountLeftOver >= 0 ? "text-green-400" : "text-red-400"}`}>
                ...
                <div className="flex flex-col gap-8 rounded-2xl border border-slate-700 bg-slate-800 p-8">
                  <div className="text-center">
                    <p className="mb-2 text-xs font-bold uppercase tracking-widest text-slate-500">Split Allocation</p>
                    <h3 className="text-sm font-black text-white">Where should your leftover flow go?</h3>
                  </div>

                  {amountLeftOver <= 0 ?
                    <div className="flex h-full items-center justify-center rounded-2xl bg-slate-900/50 p-6 text-center text-sm italic text-slate-500">
                      Please enter a monthly income and limit to see splitting options.
                    </div>
                  : <div className="flex flex-col gap-10">
                      <div className="relative pt-6">
                        <div className="absolute top-8 flex h-4 w-full overflow-hidden rounded-full bg-slate-700">
                          <div className="h-full bg-blue-500 transition-all" style={{ width: `${savingsPct}%` }} />
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <div className="rounded-2xl bg-blue-500/10 p-5 text-center transition-all hover:scale-105">
                          <TbPigMoney className="mx-auto mb-2 h-6 w-6 text-blue-400" />
                          <p className="text-[10px] font-bold uppercase tracking-widest text-blue-500">To Savings</p>
                          <p className="text-2xl font-black text-blue-400">${toSavings.toFixed(0)}</p>
                        </div>
                        <div className="rounded-2xl bg-emerald-500/10 p-5 text-center transition-all hover:scale-105">
                          <TbCoin className="mx-auto mb-2 h-6 w-6 text-emerald-400" />
                          <p className="text-[10px] font-bold uppercase tracking-widest text-emerald-500">Extra Debt Repayment</p>
                          <p className="text-2xl font-black text-emerald-400">${toDebt.toFixed(0)}</p>
                        </div>
                      </div>
                    </div>
                  }
              ...
              {mode === "onboarding" && (
                <div className="mt-12 flex items-center justify-between">
                  ...
                  <button
                    onClick={onSave}
                    disabled={saving}
                    className="flex items-center justify-center gap-2 rounded-xl bg-green-600 px-10 py-4 font-black uppercase tracking-widest text-white shadow-lg shadow-green-900/50 transition-all hover:bg-green-700 active:scale-95 disabled:opacity-50"
                  >
                    {saving ?
        ...
        {mode === "edit" && (
          <div className="sticky bottom-8 mt-12 flex flex-col gap-4">
            {error && (
              <div className="mx-auto w-full max-w-sm animate-bounce rounded-xl border border-red-500/20 bg-red-500/10 p-4 text-center text-sm font-bold text-red-400">
                {error}
              </div>
            )}
            {saved && (
              <div className="mx-auto w-full max-w-sm rounded-xl border border-emerald-500/20 bg-emerald-500/10 p-4 text-center text-sm font-bold text-emerald-400">
            ...
            <button
              onClick={onSave}
              disabled={saving}
              className="mx-auto flex w-full max-w-lg items-center justify-center gap-2 rounded-2xl bg-slate-100 px-8 py-5 text-lg font-black text-slate-900 shadow-xl transition-all hover:bg-white active:scale-95 disabled:bg-slate-700 disabled:text-slate-500"
            >
              {saving ?
